...
uint256 hashGenesisBlock("0xfc2b6a9233999036b42a6bf27813d73c18587b0ab47d9431e2c8f957b9eb5ae0");//
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Asiacoin: starting difficulty is 1 / 2^12
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+0) - GetDepthInMainChain());//此处有修改,关于币的成熟时间
}
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // nSubsidy >>= (nHeight / 840000); // Asiacoin: 840k blocks in ~4 years
//    int64 count=0,totalblocks=0,blocks10point=14400;
//    for(count=0;count<10;count++)
//    {
//        totalblocks+=(blocks10point<<count);
//        if(nHeight<totalblocks)
//            break;
//    }
//    nSubsidy>>=count;
    int nYear=nHeight/(1440*365)+1;
    if(nHeight==1)
        nSubsidy=130000000*COIN;
    else
        nSubsidy=363636.4*(11-nYear)/(1440*365)*COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30*60; // Asiacoin: 30min
static const int64 nTargetSpacing = 1 * 60; // Asiacoin:30s
static const int64 nInterval = nTargetTimespan / nTargetSpacing;//每隔多少块改变难度60
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (nActualTimespan < nTargetTimespan/4)//限制难度改变幅度
...
{
    // Asiacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Created By zaobi.org .QQ:260682605 ,欢迎咨询!2016-3-27";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd3730b1dd4fbaad3f7fdf7f675e0ac329bd7473eff1a6196a281d8d9a3dec67d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Asiacoin: increase each by adding 2 to bitcoin's value.
